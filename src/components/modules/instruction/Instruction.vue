<template>
  <div>
    ...
    <ol>
      <li>
        在选中“选择组件”（图标为手）的状态下，可以选择、拖动网格中的玩具布局。
      </li>
      <li>
        在选中网格中的一个组件实例时，可以通过工具栏调整选中的组件的旋转方向以及大小，也可以点X删除组件。
      </li>
      <li>
        选中其他组件没有什么意义……（本来是想做点击放置物品的，后来改成拖动就没用了）
      </li>
      <li>
        最下面两个组件分别是左挡板和右挡板，区别是控制的按键不一样。他们不可以旋转或者改变大小。
      </li>
      <li>
        <strong
          >小球可以放多个，但是放太多可能会很卡（问就是优化的还不够）。</strong
        >
      </li>
    </ol>
    <h3 id="游玩模式">游玩模式</h3>
...
body {
    ...
    border-radius: 3px;
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, "Segoe UI", Arial, "PingFang SC", "Microsoft YaHei", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol";
    font-size: 16px;
    line-height: 1.5
}

body pre code, body code, body pre {
    font-size: 85%;
    line-height: 1.45;
...
body pre>code::before, body pre>code::after {
    content: ""
}
